...
- Newborn resuscitation: use old 18-item `/18` when `date_submitted < 2026-04-01` **and** no new-form item is TRUE (`documentation_nnr`, `q1a_gestational_age`, `shout_help_nnr`); otherwise `/46.5`. Form detect must not use `IS NULL` (Kobo `false` is not absent). Old 18-item integers are `COALESCE`d to 0 so empty old fields cannot NULL `average_score` and drop the row in cycle ranking.

- Cycle columns: each child view joins inline `skills_assessment_cohorts` (Cohort 1–3) on `date_submitted` and keeps **all** scored attempts. Output columns include `cycle_id`, `cycle_label`, `cycle_start`, `cycle_end`, `attempt_count`, `first_pass_date`, and `average_score`. `attempt_count` / `first_pass_date` are still windowed per mentee × cycle × program. Parent unions those columns. Deploy via `skills_assessment_cycle_views.sql` / `process_moh_skills_assessment_all_child_views.sql`. Flat per-attempt item columns remain only inside each child's `scored_attempts` CTE.
- DATE POLICY: `date_submitted` is the sole date for cycle assignment, old/new form scoring cutoffs, and `first_pass_date`. `date_started` / `date_ended` are passthrough attributes only and must not drive cycle or cutoff logic.
- Grain note: one row per scored attempt. Rows with NULL `average score` / NULL `mentee_id` are still excluded. Scoring fixes (NNR `date_submitted`, PIH `fix_iv_line`, etc.) are required so valid attempts get a non-null score.
