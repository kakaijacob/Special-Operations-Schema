...
- Newborn resuscitation: use old 18-item `/18` when `date_submitted < 2026-04-01` **and** any classic 18-item integer is present; otherwise `/46.5` (covers post-cutoff and pre-cutoff new-form-only rows). Old 18-item integers are `COALESCE`d to 0 so sparse nulls cannot NULL `average_score`.
