...
- Newborn resuscitation: use new `/46.5` formula unless `date_submitted <= 2026-04-01` **and** new-form fields are absent (`documentation_nnr`, `q1a_gestational_age`, `shout_help_nnr`). Old 18-item integers are `COALESCE`d to 0 so empty old fields cannot NULL `average_score` and drop the row in cycle ranking.
