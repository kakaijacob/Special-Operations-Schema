...
- DATE POLICY: `date_submitted` is the sole date for cycle assignment, old/new form scoring cutoffs, ranking tie-breaks, and `first_pass_date`. `date_started` / `date_ended` are passthrough attributes only and must not drive cycle or cutoff logic.
